...
class ScanNetEval(object):
    ...
    def evaluate_box(self, pred_list, coords_list, sem_list, ins_list):
        """
        Args:
            pred_list:
                for each scan:
        ...
        """
        pool = mp.Pool()
        results = pool.starmap(self.assign_boxes_for_scan, zip(pred_list, coords_list, sem_list, ins_list))
        pool.close()
        pool.join()
