...
class Criterion(nn.Module):
    ...
    def get_layer_loss(self, layer, aux_outputs, insts, sp_coords, sp_rgb_feats, sp_prob_labels, batch_offsets):
        ...
        # for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores, insts, indices):
            ...
            levelset_loss = levelset_loss + self.levelset_loss(sp_coords[b_s:b_e], sp_rgb_feats[b_s:b_e], pred_mask, tgt_box)
    ...
    def forward(self, pred, insts):
        ...
        if mask_kl_varzero.sum() > 0:
            loss_kl_varzero = (torch.exp(sp_logvar_preds[mask_kl_varzero]) -1)**2 + (sp_mu_preds[mask_kl_varzero] - sp_mu_labels[mask_kl_varzero])**2
        ...
        if mask_kl_var.sum() > 0:
            loss_kl_var = (sp_logvar_preds[mask_kl_var] - torch.log(sp_var_labels[mask_kl_var])) + \
                        ((sp_mu_preds[mask_kl_var] - sp_mu_labels[mask_kl_var])**2 + sp_var_labels[mask_kl_var]**2)  * (torch.exp(-2*sp_logvar_preds[mask_kl_var])) \
                        - 0.5
        ...
        if 'aux_outputs' in pred:
            for i, aux_outputs in enumerate(pred['aux_outputs']):
                loss_i, loss_out_i = self.get_layer_loss(i, aux_outputs, insts, sp_coords, sp_rgb_feats, sp_prob_labels, batch_offsets)
                loss += loss_i
