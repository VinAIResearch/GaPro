...
class SPFormer(nn.Module):
    ...
    @cuda_cast
    def loss(self, scan_ids, voxel_coords, p2v_map, v2p_map, spatial_shape, feats, coords_float, insts, superpoints, prob_labels, mu_labels, var_labels, batch_offsets, **kwargs):
        ...
        sp_feats, sp_coords, sp_rgb_feats, sp_prob_labels, sp_mu_labels, sp_var_labels = self.extract_feat(input, voxel_coords_float, voxel_rgb_feats, superpoints, p2v_map, 
    ...
    @cuda_cast
    def predict(self, scan_ids, voxel_coords, p2v_map, v2p_map, spatial_shape, feats, coords_float, insts, superpoints, prob_labels,
                batch_offsets, **kwargs):
        batch_size = len(batch_offsets) - 1
        voxel_feats = pointgroup_ops.voxelization(feats, v2p_map)
    ...
    def extract_feat(self, x, coords_float, rgb_feats, superpoints, v2p_map, prob_labels=None, mu_labels=None, var_labels=None):
        # backbone
        x = self.input_conv(x)
        x, _ = self.unet(x)
