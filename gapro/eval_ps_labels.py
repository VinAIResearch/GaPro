...
from gen_ps_utils import  getInstanceInfo
...
def get_miou_scene2(semantic_label, instance_label, ps_semantic_label, ps_instance_label, ps_instance_label_onehot, repeat=1):
    ...
    instance_label_onehot = F.one_hot(instance_label.long(), num_classes=n_inst+1).permute(1,0)[1:, :] # remove < 0 label
...
def get_scene_sem_conf(semantic_label, ps_semantic_label, num_classes=19):
    ...
    ps_semantic_label[ps_semantic_label == -100] = torch.where(semantic_label[ps_semantic_label == -100] < 18, semantic_label[ps_semantic_label == -100] +1, semantic_label[ps_semantic_label == -100] -1)
...
def main():
    ...
    for scene in tqdm(scenes):
        _, _, semantic_label, instance_label = torch.load(os.path.join(TRUE_LABELS_FOLDER, f'{scene}_inst_nostuff.pth'))
        ...
        if not os.path.exists(os.path.join(PS_LABLES_FOLDER, f'{scene}.pth')): continue
